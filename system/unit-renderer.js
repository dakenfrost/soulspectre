(function () {
    ...
    /* ── 5. Navigation & Sidebar Injection ───────────────────────────── */
    ...
    if (data.breadcrumbs && data.breadcrumbs.length) {
        var crumbs = data.breadcrumbs.map(function (bc, i) {
            ...
            return isLast ? '<span>' + esc(bc.label) + '</span>' : '<a href="' + esc(bc.href || '#') + '">' + esc(bc.label) + '</a>';
        }).join(' &rsaquo; ');
    ...
    /* ── 8. Main content (STABLE SPLIT LAYOUT) ────────────────────────── */
    ...
    if (data.bio) {
        var bioBody = '';
        if (data.bio.quote) bioBody += '<div class="quote-box"><p class="quote-text"><em>&ldquo;' + esc(data.bio.quote) + '&rdquo;</em></p></div>';
        (data.bio.paragraphs || []).forEach(function (p) { bioBody += '<p>' + p + '</p>'; });
        bioHtml = '<div class="hero-bio"><h2>' + esc(data.bio.title || 'Biography') + '</h2>' + bioBody + '</div>';
    }

    /* Stats Mapping */
    var STAT_MAP = [['tier','Tier','tier'],['hp','Hit Points','hp'],['atk','Attack','atk'],['ini','Initiative',''],['immunity','Immunity','special'],['protection','Protection',''],['dodge','Dodge','']];
    var statsHtml = '';
    if (data.stats) {
        STAT_MAP.forEach(function (row) {
            var key = row[0], label = row[1], cls = row[2], val = data.stats[key];
            if (!val) return;
            statsHtml += '<div class="stat-item"><span class="stat-label">' + label + '</span><span class="stat-value' + (cls ? ' ' + cls : '') + '">' + val + '</span></div>';
        });
    }
    ...
    var combatHtml = combatImgs.map(function (src, i) {
        return '<img src="' + esc(src) + '" alt="' + esc(data.name) + '"' + (i > 0 ? ' class="combat-anim"' : '') + '>';
    }).join('');
    ...
    if (data.skills && data.skills.length) {
        var skillsHtml = data.skills.map(function (sk) {
            ...
            return '<div class="skill-item sk-' + type + '">' + (sk.icon ? '<img src="' + esc(sk.icon) + '" alt="" class="skill-icon">' : '') + '<div class="skill-info"><div class="skill-header"><h4 class="skill-name">' + esc(sk.name || '') + '</h4><span class="skill-tag ' + type + '">' + type + '</span></div><p class="skill-desc">' + (sk.desc || '') + '</p></div></div>';
        }).join('');
    ...
    if (data.gallery && data.gallery.length) {
        var galleryItems = data.gallery.map(function (src) { return '<div class="carousel-item"><img src="' + esc(src) + '" alt=""></div>'; }).join('');
        main.innerHTML += '<div class="portrait-gallery-module"><h2 class="gallery-header">Gallery</h2><div class="carousel-wrap"><div class="carousel-track" id="carouselTrack">' + galleryItems + '</div></div></div>';
    }
    ...
    /* Lightbox Overlay */
    inject('<div class="lightbox-overlay" id="lightbox"><button class="lightbox-close" id="lightbox-close">&times;</button><img src="" alt="" class="lightbox-img" id="lightbox-img"></div>');
    
    document.body.appendChild(main);

    /* Logic (Carousel & Lightbox) */
    ...
    document.querySelectorAll('.carousel-item img').forEach(function (img) {
        img.addEventListener('click', function () { lImg.src = this.src; lightbox.classList.add('active'); });
    });
    if (lightbox) {
